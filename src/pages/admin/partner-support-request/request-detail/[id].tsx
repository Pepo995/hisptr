...
const TicketDetails: NextPageWithLayout = () => {
  ...
  const getTicketDetails = async (id) => {
    ...
    separateUserTrail(res?.data?.data?.ticket?.trails)?.map(
      async (ele) =>
        await dispatch(adminTicketReadApiCall({ id: ele.id, ticket_id: ele.ticket_id })),
    );
  };

  /* Checking if the location state is undefined or not. If it is undefined then it will go back to the
  previous page. If it is not undefined then it will call the getTicketDetails function. */
  useEffect(() => {
  ...
  const onStatusChangeHandler = async (e) => {
    ...
    // eslint-disable-next-line @typescript-eslint/await-thenable
    await dispatch(ticketStatusUpdateApiCall(data) as unknown as AnyAction);
        ...
        {!isLoading ? (
          <>
            ...
            <Card className="bg-white">
              <CardHeader>
                <div>
                  <Link href="/admin/partner-support-request">
