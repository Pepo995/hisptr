...
const AddMember: NextPageWithLayout = ({ callbackAdd }) => {
  ...
  const validationSchema = Yup.object({
    firstName: Yup.string().max(25, FIRST_NAME_MAX_LIMIT).required(FIRST_NAME_REQUIRED),
    lastName: Yup.string().max(25, LAST_NAME_MAX_LIMIT).required(LAST_NAME_REQUIRE),
  ...
  const addMemberDetailsHandler = async (values) => {
    ...
    // eslint-disable-next-line @typescript-eslint/await-thenable
    await dispatch(addMemberApiCall(data) as unknown as AnyAction);
    setIsLoading(false);
  };
  ...
  return (
    <div>
      ...
      <Breadcrumbs
        title="Hipstr Member"
        data={[{ title: "Hipstr Member", link: "/admin/hipstr-member" }, { title: "Add Member" }]}
      />
