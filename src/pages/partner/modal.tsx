...
import { REASON_REQUIRE, REASON_TYPE_REQUIRE } from "@constants/ValidationConstants";
...
const AvailabilityModal = ({ id, refresh, centeredModal, setCenteredModal }) => {
  ...
  const UpdateStatus = async (values) => {
    ...
    const response = await dispatch(AvailabilityStatusUpdateAPICall(data) as unknown as AnyAction);
              ...
              {({ setFieldValue, errors, touched, values }) => (
                <Form>
                  ...
                  {errors.reason_type && touched.reason_type ? (
                    <span className="text-danger error-msg">{errors.reason_type}</span>
                  ) : null}
                  {values?.reason_type === "other" && (
                    <div className="mt-1">
                      ...
                      {errors.reason && touched.reason ? (
                        <span className="text-danger error-msg">{errors.reason}</span>
                      ) : null}
                    </div>
                  )}
                  <div className="mt-2">
                    <Button color="primary" type="submit" className="custom-btn7 me-75 mb-75">
                      Submit
                    </Button>{" "}
                    <Button
                      color="primary"
