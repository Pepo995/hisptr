...
const VideoList = () => {
  ...
  const getVideos = async () => {
    ...
    const response = await dispatch(videoListingApiCall(data));
  ...
  return (
    <>
      <div className={add || edit ? "d-none " : "d-block"}>
        <Breadcrumbs
          title="Video"
          data={[{ title: "Media" }, { title: "Video List" }]}
        />
        ...
        {url && setIsOpen ? (
          <VideoModal url={url} setIsOpen={setIsOpen} open={isOpen} />
        ) : null}
      </div>
      {add ? (
        <AddVideo open={add} callbackAdd={callbackAdd} refresh={getVideos} />
      ) : null}
      {edit ? (
        <EditVideo
          open={edit}
