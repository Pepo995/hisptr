...
import ShimmerChat from "@components/ShimmerCustomer/ShimmerChat";
const ChatComponent = ({ id, permission, type }) => {
  ...
  const chatData =
    ...
    shiftFirstObject(trails)?.map((item, key) => {
      return response.user_id == item.created_by ? (
        <div className="chat-box-2 p-2 w-75 mb-1" key={key}>
          ...
          {/* <p className="sy-tx-black f-600">{FirstUpperCase(response?.user?.first_name)} {" "}{FirstUpperCase(response?.user?.last_name)}</p> */}
          ...
          <CardBody className="chat-card mt-2">
            <div id="chat-scroll">
              ...
              <div className="chat-box p-2 mt-1 mb-1 w-75">
                ...
                <p className="sy-tx-black">
                  You can expect the following response times from today’s date:
                </p>
                ...
                <p className="sy-tx-black">Looking forward to connecting with you soon.</p>
